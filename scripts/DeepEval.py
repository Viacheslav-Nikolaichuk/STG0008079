...
def create_test_cases(dataset, responses, reference_type="ground_truth"):
    ...
    for scenario in responses.get("scenarios", []):
        ...
        orig_scenario = next(
            (
                s
                for s in dataset.get("scenarios", [])
        ...
        if not orig_scenario:
            logging.warning(
                f"Scenario {scenario_id} not found in original dataset"
            )
            continue
        ...
        for question in scenario.get("questions", []):
            ...
            orig_question = next(
                (
                    q
                    for q in orig_scenario.get("questions", [])
                    if q.get("query_id") == question_id
                ),
                None,
            )
            ...
            if not orig_question:
                logging.warning(
                    f"Question {question_id} not found in scenario {scenario_id}"
                )
                continue
            ...
            for model_answer in question.get("model_answers", []):
                ...
                if not answer or answer == "Error":
                    logging.warning(
                        f"Skipping empty/error answer for {scenario_id}/{question_id}/{model_name}"
                    )
                    continue

                if reference_type == "model_answer":
                    orig_model_answer = next(
                        (
                            a
                            for a in orig_question.get("model_answers", [])
                ...
                if not reference:
                    logging.warning(
                        f"Empty reference for {scenario_id}/{question_id}/{model_name}"
                    )
                    continue
...
def evaluate_test_cases(test_cases, metrics, batch_size=15):
    ...
    # Process in batches
    for i in tqdm(
        range(0, len(test_cases), batch_size), desc="Evaluating with DeepEval"
    ):
    ...
    for result in deepeval_results:
        ...
        for test_result in test_results:
            ...
            if metrics_data and meta_idx < len(metadata):
                combined = {**metrics_data, **metadata[meta_idx]}
                combined_results.append(combined)
                meta_idx += 1

    if meta_idx < len(metadata):
        logging.warning(
            f"Only processed {meta_idx} out of {len(metadata)} metadata entries"
        )
...
def compute_aggregated_metrics(results, metric_names):
    ...
    for metric_name in metric_names:
        ...
        aggregated_metrics[metric_name] = {
            "overall": {
                "score": sum(r["score"] for r in metric_results)
                / len(metric_results),
                "pass_rate": sum(1 for r in metric_results if r["passed"])
                / len(metric_results),
            },
            "by_model": _compute_group_metrics(results, metric_name, "model"),
            "by_question_type": _compute_group_metrics(
                results, metric_name, "question_type"
            ),
            "by_difficulty": _compute_group_metrics(
                results, metric_name, "difficulty"
            ),
        }
...
def _compute_group_metrics(results, metric_name, group_key):
    ...
    for group, group_metric_results in grouped_results.items():
        group_metrics[group] = {
            "score": sum(r["score"] for r in group_metric_results)
            / len(group_metric_results),
            "pass_rate": sum(1 for r in group_metric_results if r["passed"])
            / len(group_metric_results),
        }
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate LLM responses using DeepEval"
    )
    parser.add_argument(
        "--dataset",
    ...
    parser.add_argument(
        "--responses", required=True, help="Path to LLM responses JSON file"
    )
    parser.add_argument(
        "--output-dir",
        default="Results-DeepEval",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.7,
        help="Threshold for passing metrics",
    )
    parser.add_argument(
        "--batch-size", type=int, default=15, help="Batch size for evaluation"
    )
    parser.add_argument(
        "--reference-type",
        choices=["ground_truth", "model_answer"],
    ...
    logging.info(
        f"Evaluating responses from {model_name} using {args.eval_model}"
    )
    logging.info(f"Using reference type: {args.reference_type}")

    metrics = create_metrics(
        args.metrics, threshold=args.threshold, eval_model=args.eval_model
    )

    test_cases, metadata = create_test_cases(
        dataset, responses, args.reference_type
    )
    ...
    detailed_output_path = (
        output_dir
        / f"deepeval_detailed_{model_name}_{metrics_str}_{args.reference_type}.json".replace(
            ":", "-"
        )
    )
    ...
    aggregated_output_path = (
        output_dir
        / f"deepeval_aggregated_{model_name}_{metrics_str}_{args.reference_type}.json".replace(
            ":", "-"
        )
    )
    ...
    logging.info(
        f"Successfully saved detailed results to {detailed_output_path}"
    )
    logging.info(
        f"Successfully saved aggregated results to {aggregated_output_path}"
    )
    ...
    for metric_name in args.metrics:
        ...
        for model, model_metrics in aggregated_metrics[metric_name][
            "by_model"
        ].items():
            logging.info(
                f"    {model or 'base'}: Score = {model_metrics['score']:.4f}, Pass Rate = {model_metrics['pass_rate']:.2%}"
            )

        # Log breakdown by question type
        logging.info("  Breakdown by Question Type:")
        for q_type, type_metrics in aggregated_metrics[metric_name][
            "by_question_type"
        ].items():
            logging.info(
                f"    {q_type}: Score = {type_metrics['score']:.4f}, Pass Rate = {type_metrics['pass_rate']:.2%}"
            )

        # Log breakdown by difficulty
        logging.info("  Breakdown by Difficulty:")
        for difficulty, diff_metrics in aggregated_metrics[metric_name][
            "by_difficulty"
        ].items():
            logging.info(
                f"    {difficulty}: Score = {diff_metrics['score']:.4f}, Pass Rate = {diff_metrics['pass_rate']:.2%}"
            )
