            ...
            try:
                ...
                if is_deepeval_format:
                    ...
                    for result_item in data["results"]:
                        ...
                        if question is None or answer is None or reference is None:
                            print(f"    Warning: Item missing 'question', 'answer', or 'reference'. Skipping.")
                            continue
                        if query_id is None:
                            print(f"    Warning: DeepEval item missing 'query_id'. Question: '{question[:30]}...'")
                ...
                if not score_data_list:
                    print(f"    No valid score data entries found or extracted in {filename}.")
                    continue

                lowest_items = heapq.nsmallest(num_lowest, score_data_list, key=lambda x: x['score'])
                ...
                print(f"    Found {len(score_data_list)} score data entries. Identified {len(lowest_items)} lowest.")
...
if __name__ == "__main__":
    ...
    for target_dir in DIRECTORIES_TO_SCAN:
        ...
        try:
            ...
            if dir_results:
                ...
                print(f"No results generated for directory '{target_dir}', empty report file saved to {output_path}")
    ...
    if not overall_success:
        ...
        sys.exit(1)
         ...
         print("Completed, but no results were generated across any directories (check directories and file contents).")
    else:
