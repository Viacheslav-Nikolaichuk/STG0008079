        ...
        for question in scenario.get("questions", []):
            ...
            for model_answer in question.get("model_answers", []):
                ...
                if not isinstance(answer, str) or not answer.strip():
                    logging.warning(
                        f"Invalid answer for {scenario_id}/{question_id}/{model_name}: {answer}"
                    )
                    skipped_count += 1
                    continue
                ...
                if reference_type == "model_answer":
                    ...
                    if not orig_model_answer:
                        logging.warning(
                            f"No original model answer for {scenario_id}/{question_id}/{model_name}"
                        )
                        skipped_count += 1
                        continue
                ...
                if not isinstance(reference, str) or not reference.strip():
                    logging.warning(
                        f"Invalid reference for {scenario_id}/{question_id}/{model_name}: {reference}"
                    )
                    skipped_count += 1
                    continue
...
def compute_bertscore(answers, references, batch_size=15):
    ...
    if len(valid_pairs) < len(answers):
        logging.warning(
            f"Skipped {len(answers) - len(valid_pairs)} invalid pairs in compute_bertscore"
        )
    ...
    for i in tqdm(
        range(0, len(answers), batch_size), desc="Computing BERTScore"
    ):
        ...
        all_results["precision"].extend(
            [float(p) for p in results["precision"]]
        )
...
def compute_aggregated_metrics(results, index_mapping):
    ...
    # Overall average
    overall = {
        "precision": sum(result["precision"] for result in results)
        / len(results),
    ...
    for model, data in by_model.items():
        data["precision"] = sum(r["precision"] for r in data["results"]) / len(
            data["results"]
        )
        data["recall"] = sum(r["recall"] for r in data["results"]) / len(
            data["results"]
        )
        data["f1"] = sum(r["f1"] for r in data["results"]) / len(
            data["results"]
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate LLM responses using BERTScore"
    )
    parser.add_argument(
        "--dataset",
        default="data/dataset.json",
        help="Path to original dataset with ground truth",
    )
    parser.add_argument(
    ...
    combined_results = combine_results(
        answers, references, bertscore_results, metadata
    )

    aggregated_metrics = compute_aggregated_metrics(
        combined_results, index_mapping
    )
    ...
    detailed_output_path = (
        output_dir
        / f"bertscore_detailed_{model_name}_{args.reference_type}.json"
    )
    ...
    aggregated_output_path = (
        output_dir
        / f"bertscore_aggregated_{model_name}_{args.reference_type}.json"
    )
